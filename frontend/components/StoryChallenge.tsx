...
export function StoryChallenge({ levelSlug }: { levelSlug: string }) {
  ...
  const [result, setResult] = useState<StoryOrderResult | null>(null);
  ...
  useEffect(() => {
    ...
    apiFetch<{
      ...
      data: {
        ...
        shuffledEvents: StoryEventCard[];
        levelNumber: number;
        maxLevel: number;
      };
    }>("/games/stories/sessions", { method: "POST", body: JSON.stringify({ playerId, storySlug: levelSlug }) })
      .then((response) => {
        if (!cancelled) {
          setSessionId(response.data.session.id);
          setStoryId(response.data.story.id);
          setTitle(response.data.story.title);
          setEvents(shuffle(response.data.shuffledEvents));
  ...
  async function submit() {
    if (!sessionId || !storyId || !events || isSubmitting) return;
    ...
    try {
      const response = await apiFetch<{
        ...
        method: "POST",
        body: JSON.stringify({ sessionId, storyId, orderedEventIds: events.map((event) => event.id) }),
      });
  ...
  if (result) {
    return (
      <LevelCompleteScreen
        label={result.isCorrect ? "STORY COMPLETE" : "STORY CHALLENGE"}
